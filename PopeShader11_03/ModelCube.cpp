...
bool LightCube::InitVertexBuffer( ID3D11Device * pDevice )
{
	...
	ZeroMemory( &vbd, sizeof( vbd ) );
	vbd.Usage = D3D11_USAGE_DEFAULT;
	vbd.ByteWidth = sizeof( LightVertex ) * 24;
	vbd.BindFlags = D3D11_BIND_VERTEX_BUFFER;
	vbd.CPUAccessFlags = 0;
...
bool LightCube::InitIndexBuffer( ID3D11Device * pDevice )
{
	WORD indices[] =
	{
		3,1,0,
...
UINT LightCube::GetStride() const
{
	return sizeof( LightVertex );
}
